...
namespace Pixyz.OptimizeSDK.Editor.MeshProcessing
{
	...
	public class PixyzContext : IDisposable, ICloneable
	{
		...
		/// </summary>
		private Dictionary<UnityEngine.Material, uint> _materialUnityToPixyz = new Dictionary<UnityEngine.Material, uint>();
		...
		/// </summary>
		private Dictionary<uint, UnityEngine.Material> _materialPixyzToUnity = new Dictionary<uint, UnityEngine.Material>();
		...
		/// </summary>
		private Dictionary<Texture2D, uint> _imagesUnityToPixyz = new Dictionary<Texture2D, uint>();
		...
		/// </summary>
		private Dictionary<uint, Texture2D> _imagesPixyzToUnity = new Dictionary<uint, Texture2D>();
		...
		public uint[] UnityMeshesToPixyzCreate(in MeshData meshData)
		{
			...
			{
				defs[i] = UnityMeshToPixyz(filteredData.renderers[i], filteredData.meshes[i], filteredData.materials[i], false);
			}
		...
		{
			Native.Polygonal.MeshDefinition def = UnityMeshToPixyz(renderer, mesh, materials, false);
		...
		public Native.Polygonal.MeshDefinition[] UnityMeshesToPixyz(IList<Renderer> renderers, IList<Mesh> meshes, IList<UnityEngine.Material[]> materials)
		{
			...
			{
				defs[i] = UnityMeshToPixyz(renderers[i], meshes[i], materials[i], false);
			}
		...
		/// <param name="materials"></param>
		/// <returns></returns>
		public Native.Polygonal.MeshDefinition UnityMeshToPixyz(Renderer renderer, Mesh mesh, UnityEngine.Material[] materials, bool generateMatrices = true)
		{
			if(!mesh.isReadable)
			{
		...
		private Native.Material.MaterialList GenerateMaterials(UnityEngine.Material[] materials)
		{
			...
			for (int i = 0; i < materials.Length; ++i)
			{
				...
				else
				{
					...
					defs.Add(Conversions.ConvertMaterial(material, ref _imagesUnityToPixyz));
		...
		public Renderer PixyzMeshToUnityRenderer(Native.Polygonal.MeshDefinition def)
		{
			...
			UnityEngine.Material[] materials = ExtractMaterials(def.dressedPolys);
			Renderer renderer = null;

			if(def.lines.length > 0 || def.points.length > 0)
			{
				if (_lineMaterial == null && def.lines.length > 0)
					_lineMaterial = new Material(new Material(ShaderUtilities.GetPixyzLineShader()));

				if (_pointMaterial == null && def.points.length > 0)
					_pointMaterial = new Material(new Material(ShaderUtilities.GetPixyzSplatsShader()));
