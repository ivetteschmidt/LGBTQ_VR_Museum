...
namespace Pixyz.OptimizeSDK
{
	public static partial class Conversions
	{
					...
					case MeshTopology.Triangles:
						{
							...
							dressedPoly.externalId = (uint)dressedPolies.Count + 1; // Incremental, starting from 1
		...
		public static int[][] ExtractTrianglesFromDefinition(in Native.Polygonal.MeshDefinition meshDefinition)
		{
			...
			int subMeshCount = triangles.Length + meshDefinition.lines.length;
			...
			for (int s = 0; s < triangles.Length; s++)
			{
				...
				if (meshDefinition.dressedPolys[s].triCount != 0 && meshDefinition.dressedPolys[s].quadCount != 0)
				{
					tcount = meshDefinition.dressedPolys[s].triCount * 3 + meshDefinition.dressedPolys[s].quadCount * 6;
				}
				else if (meshDefinition.dressedPolys[s].quadCount != 0)
				{
					tcount = meshDefinition.dressedPolys[s].quadCount * 6;
				}

				var striangles = triangles[s] = new int[tcount];
				for (int t = 0; t < meshDefinition.dressedPolys[s].quadCount; t++)
				{
					int offsetTri = t * 6;
					int offsetQuad = t * 4;
			...
			for (int s = 0; s < triangles.Length; s++)
			{
				...
				int tcount = meshDefinition.dressedPolys[s].triCount * 3;

				int[] striangles = null;

				if (triangles[s] == null || triangles[s].Length == 0)
					striangles = triangles[s] = new int[tcount];
				else
					striangles = triangles[s];

				for (int t = 0; t < tcount; t += 3)
				{
					striangles[t] = itris[offset + t];
					striangles[t + 1] = itris[offset + t + 1];
					striangles[t + 2] = itris[offset + t + 2];
				}
			}
		...
		public static void ConvertMeshDefinition(in Native.Polygonal.MeshDefinition meshDefinition, Mesh mesh, float scaleFactor = 1.0f)
		{
			...
			{
				mesh.SetIndices(lines[l], MeshTopology.Lines, triangles.Length + l);
			}
			...
			if (meshDefinition.points.length > 0)
			{
				...
				{
					pointsInd[i] = triangles.Length + lines.Length + i;
				}
				...
				if (meshDefinition.points.length > 0)
				{
					mesh.SetIndices(pointsInd, MeshTopology.Points, pointsIndex, false);
				}
			}
