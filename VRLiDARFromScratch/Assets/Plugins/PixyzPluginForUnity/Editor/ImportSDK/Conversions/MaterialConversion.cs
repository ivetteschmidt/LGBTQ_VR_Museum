...
namespace Pixyz.ImportSDK
{
	public static partial class Conversions
	{
		...
		private static Texture2D GetTexture(Native.Material.Texture textureExtract, ref Dictionary<uint, Texture2D> textureMap)
		{
			...
			{
				textureMap.Add(textureExtract.image, ConvertImageDefinition(Native.NativeInterface.GetImageDefinition(textureExtract.image)));
			}
		...
		public static void ConvertMaterialExtract(Native.Material.MaterialDefinition materialExtract, ref UnityEngine.Material material, ref Dictionary<uint, Texture2D> textureMap)
		{
			...
			if (materialExtract.roughness._type == Native.Material.CoeffOrTexture.Type.TEXTURE)
			{
				...
				{
					material.SetFloat("_Metallic", Mathf.Clamp((float)materialExtract.metallic.coeff, 0, 1));
				}
		...
		public static UnityEngine.Material ConvertMaterialExtract(Native.Material.MaterialDefinition materialExtract, ref Dictionary<uint, Texture2D> textureMap)
		{
			var material = new UnityEngine.Material(ShaderUtilities.GetDefaultShader()); // todo: pick right shader
			ConvertMaterialExtract(materialExtract, ref material, ref textureMap);
			return material;
		}

		public static UnityEngine.Material[] ConvertMaterialExtracts(Native.Material.MaterialDefinitionList materialExtractList, ref Dictionary<uint, Texture2D> textureMap)
		{
			var materials = new UnityEngine.Material[materialExtractList.length];
			for (int i = 0; i < materials.Length; i++)
			{
				materials[i] = ConvertMaterialExtract(materialExtractList[i], ref textureMap);
			}
			return materials;
		}
		...
		public static Native.Material.MaterialDefinition ConvertMaterial(UnityEngine.Material material, ref Dictionary<Texture2D, uint> texMap)
		{
			...
			materialExtract.name = material.name + " " + material.GetInstanceID();
