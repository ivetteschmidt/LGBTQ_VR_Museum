...
//#endif
        ...
        private Ident AutomaticImportProcess(Ident root, ImportSettings settings)
        {
            ...
            reportImportProgressed(0.3f, "Processing imported model");
            if (settings.importPoints)
            {
                if (ext == "e57" || ext == "pts" || ext == "ptx" || ext == "xyz" || ext == "rcp")
                {
                    ...
                    {
                        IntList qualities = new IntList(settings.qualities.lods.Length);
                        for (int i=0;i< settings.qualities.lods.Length;i++)
                        {
                            qualities[i] = (int)settings.qualities.lods[i].quality;
                        }
                            
                        //Generate LOD
                        NativeInterface.GenerateLOD(rootList,  qualities);
                    }
                    else
                    {
                    ...
                    var firstChild = NativeInterface.GetOccurrenceChildren(root).list.First();
                    return firstChild;
                }
            }
            else
            ...
            if(settings.combinePatchesByMaterial && ext!="pxz")
            {
